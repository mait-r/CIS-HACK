...
module.exports = {
  ...
  testInput: (query) => {
    ...
    if (query[2].length !== 4) {
      // Checking if time is not in HHMM format
      return false;
    }
    if (query[3] && query[3] !== "today") {
      // Checking if date-month is not in DD/MM format
    ...
    if (!query[3] || query[3] === "today") {
      ...
      const userString = new Date(
        year, month, date, hour, minutes, 0, 0
      ).toISOString();
      const userEpoch = Date.parse(userString);
      if (userEpoch > currEpoch) {
        // Checking if user input not in past
        return true;
      }
    } else {
      const year = istString.slice(0, 4);
